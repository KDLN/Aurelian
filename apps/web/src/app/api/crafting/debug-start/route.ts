...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      // DEBUG: Force 2-second completion time
      const eta = new Date(Date.now() + 2000);
      ...
      return {
        ...
        estimatedCompletionTime: 2 / 60 // 2 seconds in minutes
      };
    });

    return NextResponse.json({
      success: true,
      message: `Started DEBUG crafting ${quantity}x ${result.craftJob.blueprint.output.name} (2 seconds)`,
    ...
    console.error('Start debug crafting error:', error);
    return NextResponse.json({
      error: 'Failed to start debug crafting',
