...
export default function ContractsAE2(){
  ...
  function add(){
    const item=(document.getElementById('k_item') as HTMLSelectElement).value;
    const qty= Number((document.getElementById('k_qty') as HTMLInputElement).value||100);
    const price= Number((document.getElementById('k_price') as HTMLInputElement).value||8);
    world.addContract(item, qty, price);
  }
  return (
    <div className="ae wrap">
      <div className="panel left">
        <h1>Contracts</h1>
        <div>
          <select id="k_item">{Object.keys(world.warehouse).map(k=><option key={k}>{k}</option>)}</select>
          <input id="k_qty" type="number" defaultValue={100} />
          <input id="k_price" type="number" defaultValue={8} />
          <button className="btn" onClick={add}>Add</button>
        </div>
        <div className="muted" style={{marginTop:8}}>Gold: <span className="pill">{world.gold}</span></div>
      </div>
      <div className="panel right">
        <h2>Active</h2>
        <div>{world.contracts.map(c=>(<div className="card" key={c.id}><b>{c.item}</b> — {c.qty}/day @ {c.price}g</div>))}</div>
      </div>
