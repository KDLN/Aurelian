...
export default function ScoutsAE2(){
  ...
  function send(){
    const item=(document.getElementById('s_item') as HTMLSelectElement).value;
    const qty= Number((document.getElementById('s_qty') as HTMLInputElement).value||10);
    const risk=(document.getElementById('s_risk') as HTMLSelectElement).value as any;
    const dur=[2,3,4,5,6][Math.floor(Math.random()*5)];
    world.sendMission(item, qty, risk, dur);
  }
  function etaText(eta:number){
    const minutes = ((window as any).__ae_day??1 - 1) * 1440 + ((window as any).__ae_min??360);
    const left = Math.max(0, Math.floor((eta - minutes)/60));
    return left + "h";
  }
  return (
    <div className="ae wrap">
      <div className="panel left">
        <h1>Scouts</h1>
        <div className="muted">Sends procurement orders; returns later today.</div>
        <div>
          <select id="s_item">{Object.keys(world.warehouse).map(k=><option key={k}>{k}</option>)}</select>
          <input id="s_qty" type="number" defaultValue={50} />
          <select id="s_risk"><option>Low</option><option>Medium</option><option>High</option></select>
          <button className="btn" onClick={send}>Send</button>
        </div>
      </div>
      <div className="panel right">
        <h2>Active Missions</h2>
        <div>{world.missions.map(m=>(<div className="card" key={m.id}>
          <b>{m.item}</b> x{m.qty} — {m.risk} | ETA {etaText(m.eta)}
          <div className="muted">Progress: {m.progress}%</div>
        </div>))}</div>
      </div>
