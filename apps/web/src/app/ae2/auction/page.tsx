...
export default function AuctionAE2(){
  ...
  const items = Object.keys(world.warehouse);
  function list(){ 
    const item = (document.getElementById('a_item') as HTMLSelectElement).value;
    const qty  = Number((document.getElementById('a_qty') as HTMLInputElement).value||1);
    const price= Number((document.getElementById('a_price') as HTMLInputElement).value||1);
    if(!world.list(item, qty, price)) alert('Not enough stock.');
  }
  return (
    <div className="ae wrap">
      <div className="panel left">
        <h1>Auction</h1>
        <div>Gold: <span className="pill">{world.gold}</span></div>
        <div className="muted">Auto-sells near index; returns after age.</div>
        <div>
          <select id="a_item">{items.map(k=> <option key={k} value={k}>{k}</option>)}</select>
          <input id="a_qty" type="number" defaultValue={10} />
          <input id="a_price" type="number" defaultValue={10} />
          <button className="btn" onClick={list}>List</button>
        </div>
        <h2>Warehouse</h2>
      ...
      <div className="panel right">
        <h2>Listings</h2>
        <div>{world.listings.map(l=>(<div className="card" key={l.id}><b>{l.item}</b> x{l.qty} @ {l.price}g <span className="muted">age {l.age}</span></div>))}</div>
      </div>
